#!/usr/bin/env bash
# build.sh — Build AcreetionOS-GNOME ISO
# Usage: ./build.sh
...
echo "  :: AcreetionOS-GNOME — Building ISO"
...
FOUND_ISO=$(find "${OUTPUT_DIR}" -name "AcreetionOS-GNOME*.iso" -type f 2>/dev/null | head -1 || true)
if [ -n "$FOUND_ISO" ]; then
  echo "  ✓ ISO: $FOUND_ISO"
